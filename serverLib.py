...
from classesLib import MacroParams, Page, Resource

class PHFSServer(TplInterpreter):
    class VFSRequest(Request):
    ...
    def wsgi(self, environ, start_response):
        ...
        request = self.VFSRequest(environ, resource.path_virtual, resource.path_real)
        if path[0:2] == '/~':
            section = path[2:]
            ...
            if path[-1] == '/' and resource.path_real[-1] == '/':
                page = self.get_section('', MacroParams([], self, {}, request, self.vfs_manager), True, True)
                response = Response(page.content, page.status, page.headers, mimetype=mimeLib.getmime(path))
            else:
                if os.path.exists(resource.path_real) and os.path.isfile(resource.path_real):
                    response = send_file(resource.path_real, environ)
        else:
